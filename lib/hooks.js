...
module.exports.encrypt = function(keypair) {
  return function(buffer, contact,  next) {
    ...
    //TODO--move this to separate hook, for encryption we just need a valid pubkey
    if ((!contact.pubkey) || (contact.pubkey.length !== 33)) {
      return next(new Error('Invalid public key'));
    }
