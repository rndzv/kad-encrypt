...
/**
 * Used as a transport.before('send') hook
 * #encrypt
 * @param {Object} keypair (optional)
 */
module.exports.encrypt = function(keypair) {
  return function(buffer, contact,  next) {
    //throw error if contact pubkey does not generate node ID
    if (utils.getNodeIdFromPublicKey(contact.pubkey) !== contact.nodeID) {
      next(new Error('NodeID does not match public key'))
    }
    ...
    const ciphertext = keypair.encrypt(contact.pubkey, iv, plaintext)

    //now assign 'buffer' to a new buffer
    buffer = Buffer.alloc(iv.length + ciphertext.length)
    iv.copy(buffer, 0)
    ciphertext.copy(buffer, iv.length)
...
/**
 * Used as a transport.before('deserialize') hook
 * #decrypt
...
module.exports.decrypt = function(keypair) {
  return function(buffer,next) {
    ...
    //then try to decrypt
    try {
      const plaintext = keypair.decrypt(iv,ciphertext)
    } catch (err) {
