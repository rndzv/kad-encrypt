...
KeyPair.prototype.encrypt = function(pubkey, iv, plaintext) {
  ...
  const myPubKey = Buffer.from(this._keypair.getPublicKey(),'hex');

  var ciphertext = Buffer.alloc(myPubKey.length + tag.length +
                                updatetext.length + finaltext.length)
  var buffPosition = 0;
  buffPosition += myPubKey.copy(ciphertext, buffPosition)
  buffPosition += tag.copy(ciphertext, buffPosition)
  buffPosition += updatetext.copy(ciphertext, buffPosition)
  buffPosition += finaltext.copy(ciphertext, buffPosition)
...
KeyPair.prototype.decrypt = function(iv, ciphertext) {
  //split packet
  const pubkey = Buffer.from(ciphertext.slice(0,32))
  const tag = Buffer.from(ciphertext.slice(32,48))
  const text = Buffer.from(ciphertext.slice(48))
